...
MapParticlesByDay <- function(df, outpath, TGL = 1){
  ...
  for(i in 1:length(var_fact)){
    ...
    if(is.null(TGL)){
      sub_df <- subset(df, df$age == var_fact[i])
    }else{
      sub_df <- subset(df, df$age == var_fact[i] & df$TGL == 1)
    }
    ...
    PNG <- paste0(dirpath,'trajectories/', '/ParticlesAge',number ,'.png')
    ...
    mtext(text = paste('Day', (var_fact[i])), side = 3, adj = 0.05, line = -1, font = 2)
