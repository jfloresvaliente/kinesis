...
weight_map_byage <- function(
  ...
  if(alive == T){
    ...
    outname <- 'weightMapAgeAlive'
  }else{
    outname <- 'weightMapAge'
  }
  
  for(i in 1:length(ages)){
    sub_df <- subset(df, df$age == ages[i] & df$TGL == 1)
    
    PNG <- paste0(outpath, outname, ages[i],'.png')
    map <- ggplot(data = sub_df, aes(x = lon, y = lat))
    map <- map +
      geom_point(data = sub_df, aes(x = lon, y = lat, colour = Wweight), size = 1)+
      scale_colour_gradientn(colours = tim.colors(64), limits = zlimmap, expression('Weight'))+
